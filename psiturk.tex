...
Finally, the psiturk.org cloud based component includes a hosted ``ad server" 
...
\subsection{Web application server (webserver + database)}
...
\subsection{psiturk.org Cloud-based Services}
...
in worker's browsers unless they are encrypted using SSL.  The psiturk.org
ad server allows users of the psiturk system to host ads on the psiturk.org
website which are encrypted.  These ads are what appear in the browseable listing
...
easily downloaded.  The coordination of these downloads is handled by the psiturk.org cloud.
...
\hl{What are the requirements for using psiTurk}
